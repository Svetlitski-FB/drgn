		     ...
		     struct drgn_elf_file **file_ret)
{
	...
	*abstract_origin_ret = group->die_addr;
	return drgn_abstract_origin_file(prog, group->die_addr, file_ret);
}
				      ...
				      Dwarf_Die *die_ret)
{
	...
	if (arg.found) {
		...
		module = *userdatap;
		file = module->debug_file;
	} else {
		uintptr_t abstract_origin;
		struct drgn_error *err = find_abstract_origin(prog, name, &abstract_origin, &file);
		if (err)
			return err;
		err = drgn_dwarf_index_find_die(abstract_origin, file, &die);
		if (err)
			return err;
	}
	...
	struct drgn_error* err = NULL;
	if (ret)
		err = drgn_type_from_dwarf(prog->dbinfo, file, &die, ret);
	if (err)
		return err;
	if (die_ret)
		*die_ret = die;
